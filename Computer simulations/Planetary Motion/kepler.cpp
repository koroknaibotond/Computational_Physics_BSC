...
const double GmPlusM = 39.4769;

// Correction in force
const double alpha = 1.1e-8; // AU^2

bool switch_t_with_y = false; // to interpolate to y = 0

// Variables to track major axis direction and magnitude
double major_axis_direction = 0.0; // in radians
double major_axis_magnitude = 1.0;

// Derivative vector for Newton's law of gravitation with correction
Vector f(const Vector &x)
{
    ...
    f[1] = v_x;
    f[2] = v_y;
    // Correction with alpha
    f[3] = -GmPlusM * r_x / rCubed * (1 + alpha / rSquared);
    f[4] = -GmPlusM * r_y / rCubed * (1 + alpha / rSquared);
    if (switch_t_with_y)
    {
...
int main()
{
    ...
    ofstream dataFile("fixedmercurydata2.data");
    Vector x = x0;
    ...
    do
    {
        for (int i = 0; i < 5; i++)
            dataFile << x[i] << '\t';
        // Calculate major axis direction and magnitude
        double angle = atan2(x[2], x[1]);
        major_axis_direction = angle;
        major_axis_magnitude = sqrt(x[1] * x[1] + x[2] * x[2]);
        dataFile << major_axis_direction << '\t' << major_axis_magnitude << '\n';
        double y = x[2];
        RK4Step(x, dt, f);
        ++steps;
        if (y * x[2] < 0)
